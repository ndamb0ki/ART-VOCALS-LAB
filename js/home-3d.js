...
function animate() {
  ...
  // Scroll drives a clear top-to-bottom journey instead of flipping in place.
  const startY = 2.35;
  const endY = -2.55;
  const travelY = THREE.MathUtils.lerp(startY, endY, progress);
  const float = Math.sin(time * 0.8) * 0.055 * (1 - progress);
  tubeGroup.position.y = travelY + float;

  // Continuous forward flip while the tube travels down the page.
  tubeGroup.rotation.x = progress * Math.PI * 3.2;
  tubeGroup.rotation.z = Math.sin(time * 0.42) * 0.018 * (1 - progress);

  // Ease backward in depth near the Explore section hand-off.
  const depthStart = Math.max(0, (progress - 0.72) / 0.28);
  tubeGroup.position.z = -depthStart * 1.35;

  const responsiveScale = window.innerWidth > 1100 ? 0.72 : window.innerWidth > 768 ? 0.62 : 0.48;
  tubeGroup.scale.setScalar(responsiveScale * (1 - depthStart * 0.10));
