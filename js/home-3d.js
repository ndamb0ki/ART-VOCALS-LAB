...
function animate() {
  ...
  const progress = Number(document.documentElement.style.getPropertyValue("--hero-scroll-progress")) || 0;

  // Float at rest, then progressively settle as the page scrolls.
  tubeGroup.position.y = Math.sin(time * 0.8) * 0.08 * (1 - progress);

  // Scroll-driven full flip + a restrained natural sway at the top.
  tubeGroup.rotation.x = progress * Math.PI * 2;
  tubeGroup.rotation.z = Math.sin(time * 0.45) * 0.018 * (1 - progress);

  // Pull the tube slightly away as the flip completes, creating the feeling
  // that it is travelling toward the destination frame below.
  tubeGroup.position.z = -progress * 0.65;
